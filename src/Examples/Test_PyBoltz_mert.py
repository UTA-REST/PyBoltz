...
def EditCrossSections(self):
 gd = np.load('gases.npy').item()
 KeyList = []
 for key in self.ExtraParameters.keys():
  if key.find("NewCrossSection"):
   KeyList.append(key)
 for CrossSectionKey in KeyList:
  print(CrossSectionKey)
  gd[CrossSectionKey[15:]] = self.ExtraParameters[CrossSectionKey]
...
# If you call gas61 it will use mert with the following parameters
ExtraParameters = {}
ExtraParameters['A'] = 2
