...
cpdef MIXER(Magboltz object):
    cdef double QATT[6][4000], ECHARG,JHI,JLOW,EHI,F2,BP
    ...
    for IE in range(4000):
        ...
        for KGAS in range(object.NGAS):
            if KGAS == 1:
                object.CF1[IE][NP] = MIXOBJECT.Gases[0].Q[1][IE] * object.VANN[0]
            elif KGAS == 2:
                object.CF1[IE][NP] = MIXOBJECT.Gases[1].Q[1][IE] * object.VANN[1]
            elif KGAS == 3:
                object.CF1[IE][NP] = MIXOBJECT.Gases[2].Q[1][IE] * object.VANN[2]
            elif KGAS == 4:
                object.CF1[IE][NP] = MIXOBJECT.Gases[3].Q[1][IE] * object.VANN[3]
            elif KGAS == 5:
                object.CF1[IE][NP] = MIXOBJECT.Gases[4].Q[1][IE] * object.VANN[4]
            elif KGAS == 6:
                object.CF1[IE][NP] = MIXOBJECT.Gases[5].Q[1][IE] * object.VANN[5]
            ...
            if MIXOBJECT.Gases[KGAS].NIN > 0:
                for J in range(int(MIXOBJECT.Gases[KGAS].NIN)):
                    ...
                    if IE == 0:
                        RGAS = 1 + MIXOBJECT.Gases[KGAS].E[1] / 2
                        object.RGAS1[NP] = RGAS
                        object.EIN1[NP] = MIXOBJECT.Gases[KGAS].EI[J] / RGAS
                        L = 4
    ...
    F2 = object.EFIELD * Magboltz.CONST3
    object.ELOW = object.TMAX * (object.TMAX * BP - F2 * sqrt(0.5 * object.EFINAL)) / object.ESTEP - 1
    object.ELOW = min(object.ELOW, object.SMALL)
    EHI = object.TMAX * (object.TMAX * BP + F2 * sqrt(0.5 * object.EFINAL)) / object.ESTEP + 1
    if EHI > 10000:
        EHI = 10000
    for l in range(8):
        l = I + 1
        JLOW = 4000 - 500 * (9 - I) + 1 + int(object.ELOW)
