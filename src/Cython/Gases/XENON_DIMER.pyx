...
cdef void Gas77(Gas*object):
    ...
    fact = np.linspace(4,1,15)
    fact = np.array([2, 2, 2.2, 3, 4,4,4,3.5,3,2.5,2, 1.5, 1.3, 1.2, 1])
    for i in range(15):  
        YMOM[i] = YMOM[i] * fact[i]
    #for i in range(15):  
    #    YMOM[i] = YMOM[i] * 2
    ...
    object.E[1] = 2.0 * ElectronMass / (<float> (2*131.30) * AMU)
    cdef double EOBY[12]
